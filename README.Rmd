...
```
The package operates with z-scores, centiles, and values. For this package, a
z-score is defined as the number of standard deviations from the mean for an
observation taken from a given distribution of values. A centile represents
similar information, but instead denotes the proportion of observations from the
observation which are above/below that centile. Z-scores and centiles can be
converted between directly in normally distributed data - some value with a
z-score of zero would be at the mean, and therefore be the value of the 50th
centile.
