make_ig_nbs_list <- function(raw_stem, x, y) {
  ...
  make_zscore_tbl <- function(sex) {
    tbl <- read.csv(file = file.path(data_dir, paste0(raw_stem, sex,
                    ...
                    header = F, sep = " ") |>
      as.data.frame() |>
      dplyr::rename(gest_age = V1, SD3neg = V2, SD2neg = V3, SD1neg = V4,
  ...
  make_percentile_tbl <- function(sex) {
    tbl <- read.csv(file = file.path(data_dir,
                    ...
                    sep = " ") |>
      as.data.frame() |>
      dplyr::rename(gest_age = V1, P03 = V2, P05 = V3, P10 = V4, P50 = V5,
...
make_ig_nbs_bodycomp_list <- function(raw_stem, x, y) {
  ...
  make_percentile_tbl <- function(sex) {
    tbl <- read.csv(file = file.path(data_dir,
                    ...
                    sep = ",") |>
      as.data.frame() |>
      dplyr::rename(gest_age = V1, P03 = V2, P10 = V3, P50 = V4, P90 = V5,
                    P97 = V6)
   return(tbl)
  }
