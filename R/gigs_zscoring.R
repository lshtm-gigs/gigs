...
#'     - Postnatal: Uncorrected WHO Child Growth standards
#' @note
#'   This function expects vectors of the same length, and will fail if these
#'   are not provided.
#' @rdname gigs_waz
#' @export
...
#' @rdname gigs_waz
#' @param length_cm Numeric vector with lengths in cm.
#' @export
gigs_laz <- function(length_cm, gest_days, age_days, sex) {
  stop_if_lengths_unequal(length_cm, gest_days, age_days, sex)
...
#' @rdname gigs_waz
#' @export
gigs_wlz <- function(weight_kg, length_cm, gest_days, age_days, sex) {
  stop_if_lengths_unequal(weight_kg, length_cm, gest_days, age_days, sex)
  ...
  z_ig_png <- fn_on_subset(ig_png_wfl_value2zscore, gigs_lgls[["ig_png"]],
                           weight_kg, length_cm, sex)
  z_who_gs_wfh <- fn_on_subset(who_gs_wfl_value2zscore, gigs_lgls[["who_gs"]],
                               weight_kg, length_cm, sex)
  z_who_gs_wfl <- fn_on_subset(who_gs_wfl_value2zscore, gigs_lgls[["who_gs"]],
                               weight_kg, length_cm, sex)

  z_out <- rep(NA_real_, length(weight_kg))
  z_out[gigs_lgls[["ig_png"]]] <- z_ig_png
  z_out[gigs_lgls[["who_gs"]] & age_days < 731] <- z_who_gs_wfh
  z_out[gigs_lgls[["who_gs"]] & age_days >= 731] <- z_who_gs_wfl
  z_out
}
...
gigs_xaz_lgls <- function(gest_days, age_days) {
  ...
  # If we change to applying WHO GS correction for preterms after 64 wks PMA
  # ga_correction_days <- 40 * 7

  # Booleans for which standard to use
  use_ig_nbs <- age_days == 0
  use_ig_png <- age_days > 0 & !is_term & inrange(pma_weeks, c(24, 64))
...
fn_on_subset <- function(gigs_fn, lgl, in1, in2, in3) {
  gigs_fn(in1[lgl], in2[lgl], in3[lgl])
}
