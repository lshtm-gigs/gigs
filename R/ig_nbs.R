...
#' @param gest_age Gestational age(s) in weeks. Must be between `33` and `42 + 6/7`.
...
#' gigs:::ig_nbs_msnt(gest_age = 40, sex = "M", acronym = "wfga")
#'
#' @rdname ig_nbs_msnt
#' @keywords internal
ig_nbs_msnt <- function(gest_age, sex, acronym) {
  new_df <- data.frame(gest_age = round(gest_age * 7),
                       sex = sex,
                       acronym = acronym,
...
#' @param gest_age Gestational age(s) in weeks. Must be between `24` and `42 + 6/7` for weight/length/head
#' circumference-for-gestational age, or from `38` to `42` for body composition equations.
#' @param sex Sex(es), either `"M"` (male) or `"F"` (female).
...
#' ig_nbs_percentile2value(p = p, gest_age = 40, sex = "M", acronym = "wfga") |>
#'   round(digits = 2)
#'
#' # Or z-scores to values
#' z <- qnorm(p)
#' ig_nbs_zscore2value(z = z, gest_age = 40, sex = "M", acronym = "wfga") |>
#'   round(digits = 2)
#'
#' # Specify which standard to use with the acronym parameter...
#' ig_nbs_zscore2value(z = z, gest_age = 40, sex = "M", acronym = "lfga") |>
#'   round(digits = 2)
#'
#' # ... or by using a standard-specific function
#' ig_nbs_lfga_zscore2value(z = z, gest_age = 40, sex = "M") |>
#'   round(digits = 2)
#'
...
#'   round(digits = 2)
#'
#' # Bad inputs will not stop the function but will instead return NA - here 20 weeks gest_age
#' # is outside the bounds of the INTERGROWTH-21st newborn size standards
#' ig_nbs_hcfga_zscore2value(z = z,
#'                           gest_age = c(20, 26, 32, 38),
#'                           sex = "F") |>
#'   round(digits = 2)
...
ig_nbs_percentile2value <- function(p, gest_age, sex, acronym) {
  ...
  fromLM_p2v <- function(max_len_vec_li) {
    ...
    max_len_vec_li$p[max_len_vec_li$gest_age < 38 | max_len_vec_li$gest_age > 42] <- NA
    lm_out <- ifelse(
      ...
      no = body_comp$y_intercept + body_comp$ga_coeff * max_len_vec_li$gest_age + qnorm(max_len_vec_li$p) * body_comp$std_dev)
    ifelse(lm_out <= 0, yes = NA, no = lm_out)
  }
  
  fromWLR_p2v <- function(max_len_vec_li) {
    wlr <- ig_nbs_wlr(gest_age = max_len_vec_li$gest_age, sex = max_len_vec_li$sex)
    wlr_out <- ifelse(
      max_len_vec_li$sex == "U",
      yes = mean_if_sex_undefined(fn = ig_nbs_percentile2value, arg1 = max_len_vec_li$p,
                                  x_arg = max_len_vec_li$gest_age, acronym = max_len_vec_li$acronym),
  ...
  out <- ifelse(test = input$gest_age >= 33,
...
#' ig_nbs_value2percentile(y = 3.12, gest_age = 40, sex = "M", acronym = "wfga") |>
#'   round(digits = 2)
#'
#' # Or values to z-scores
#' ig_nbs_value2zscore(y = 3.12, gest_age = 40, sex = "M", acronym = "wfga") |>
#'   round(digits = 2)
#'
#' # Specify which standard to use with the acronym parameter...
#' ig_nbs_value2zscore(y = 48.84, gest_age = 40, sex = "F", acronym = "lfga") |>
#'   round(digits = 2)
#'
#' # ... or by using a standard-specific function
#' ig_nbs_lfga_value2zscore(length_cm = 48.84, gest_age = 40, sex = "F") |>
#'   round(digits = 2)
#'
#' # Inputs are recycled to the input of the longest length
...
#'   round(digits = 2)
#'
#' # Bad inputs will not stop the function but will instead return NA - here 43 weeks gest_age
#' # is outside the bounds of the INTERGROWTH-21st newborn size standards
#' ig_nbs_hcfga_value2percentile(headcirc_cm = c(23.0, 28.0, 33.0, 35.0),
#'                               gest_age = c(24, 31, 38, 43),
#'                               sex = "F") |>
#'   round(digits = 2)
  ...
  fromLM_v2p <- function(max_len_vec_li) {
    ...
    ifelse(
      ...
      yes = mean_if_sex_undefined(fn = ig_nbs_value2percentile, arg1 = max_len_vec_li$y,
                                  x_arg = max_len_vec_li$gest_age, acronym = max_len_vec_li$acronym),
      no = pnorm((max_len_vec_li$y - body_comp$y_intercept - body_comp$ga_coeff * max_len_vec_li$gest_age) / body_comp$std_dev)
    )
  }
