...
#'   * `"handle_undefined_data"` - How should gigs handle undefined (`NaN`, `Inf`,
#'       `-Inf`) elements?
...
#'
#'   This argument will not accept an integer value in place of a logical value.
...
#'     encountered.
#' @returns Invisibly returns either a single character variable with the
#'   current or new value of `option`, depending on which of [gigs_option_get()]
#'   or [gigs_option_set()] is called. If `silent = FALSE` (default), then an
#'   informative message will be printed to the console regarding the
#'   current/new value of `option`.
#' @examples
#' # Show the names of all available options
#' names(.gigs_options)
#'
#' # Retrieve the value of a gigs option (with `silent =  FALSE`, would print a
#' # message)
...
#' gigs_option_set("handle_undefined_data", "warn", silent = TRUE)
#' @name gigs_options
#' @export
...
#'     gigs handle elements of `p` that are not between `0` and `1`?
#'
#'   By default, each of these options will be set to `"warn"`, so you will be
#'   informed of any invalid data you use as inputs. You can use
#'   `gigs_options_set()` to change this behaviour.
#' @seealso The `gigs_option_get()` and `gigs_option_set()` functions, which can
#'   be used to edit the values in `.gigs_options`.
#' @examples
