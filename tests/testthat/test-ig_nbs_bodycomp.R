test_percentile_tbls <- function(sex, age_lower, age_upper, acronym, tolerance) {
  get_gest_days <- function(lower, upper) {
    ga <- gigs::ig_nbs$fmfga$male$percentiles$gest_days
      ...
      vapply(
        ...
        FUN = function(x) {
          fn <- switch(x,
                       ...
                       "bfpfga" = ig_nbs_bfpfga_percentile2value,
                       "ffmfga" = ig_nbs_ffmfga_percentile2value)
          vals <- suppressWarnings(fn(sex = sex, gest_days = age, p = percentiles))
...
test_zscore_tbls <- function(sex, age_lower, age_upper, acronym, tolerance) {
  get_gest_dayss <- function(lower, upper) {
    ga <- gigs::ig_nbs$fmfga$male$percentiles$gest_days
    ga[which(ga >= lower & ga <= upper)]
  }
  tbl_names <- c("P03", "P10", "P50", "P90", "P97")
  ...
  tbl <- lapply(X = qnorm(c(0.03, 0.1, 0.5, 0.9, 0.97)),
                FUN = function(x) {
                  fn <- switch(acronym,
                   ...
                   round2(fn(sex = sex, gest_days = get_gest_dayss(age_lower, age_upper), z = x),
                         digits = digits)
                }) |>
    do.call(what = cbind) |>
    as.data.frame()
  names(tbl) <- tbl_names
  tbl$gest_days <- get_gest_dayss(age_lower, age_upper)
  pkg_tbl <- tbl[, c(ncol(tbl), 1:(ncol(tbl) - 1))]
  sex_ <- ifelse(sex == "M", yes = "male", no = "female" )
  ref_tbl <- gigs::ig_nbs[[acronym]][[sex_]]$percentiles
  expect_true(all(abs(ref_tbl - pkg_tbl) <= tolerance, na.rm = T))
...
testthat_v2x <- function(y, gest_days, sex, acronym, z_or_p = "zscores") {
  out_z_or_p <- switch(z_or_p,
         "zscores" = switch(acronym,
                            "fmfga"  = ig_nbs_fmfga_value2zscore(fat_mass_g = y, gest_days = gest_days, sex = sex),
                            "bfpfga"  = ig_nbs_bfpfga_value2zscore(body_fat_perc = y, gest_days = gest_days, sex = sex),
                            "ffmfga" = ig_nbs_ffmfga_value2zscore(fatfree_mass_g = y, gest_days = gest_days, sex = sex)),
         "percentiles" = switch(acronym,
                            "fmfga"  = ig_nbs_fmfga_value2percentile(fat_mass_g = y, gest_days = gest_days, sex = sex),
                            "bfpfga"  = ig_nbs_bfpfga_value2percentile(body_fat_perc = y, gest_days = gest_days, sex = sex),
                            "ffmfga" = ig_nbs_ffmfga_value2percentile(fatfree_mass_g = y, gest_days = gest_days, sex = sex)))
  out_value <- switch(z_or_p,
         "zscores" = switch(acronym,
                            "fmfga"  = ig_nbs_fmfga_zscore2value(z = out_z_or_p, gest_days = gest_days, sex = sex),
                            "bfpfga"  = ig_nbs_bfpfga_zscore2value(z = out_z_or_p, gest_days = gest_days, sex = sex),
                            "ffmfga" = ig_nbs_ffmfga_zscore2value(z = out_z_or_p, gest_days = gest_days, sex = sex)),
         "percentiles" = switch(acronym,
                            "fmfga"  = ig_nbs_fmfga_percentile2value(p = out_z_or_p, gest_days = gest_days, sex = sex),
                            "bfpfga"  = ig_nbs_bfpfga_percentile2value(p = out_z_or_p, gest_days = gest_days, sex = sex),
                            "ffmfga" = ig_nbs_ffmfga_percentile2value(p = out_z_or_p, gest_days = gest_days, sex = sex)))
...
test_that("Conversion of values to percentiles works", {
  perc <- "percentiles"
  # Fat mass for gestational age
  testthat_v2x(y = c(2.65, 3.00, 2.86, 3.10, 3.32), gest_days = 7 * 41, sex = "M", acronym = "fmfga", z_or_p = perc)
  testthat_v2x(y = c(2.65, 3.00, 2.86, 3.10, 3.32), gest_days = 7 * 38, sex = "F", acronym = "fmfga", z_or_p = perc)

  # Fat-free mass for gestational age
  testthat_v2x(y = c(2041.9, 2193.8, 2345.6, 2497.3, 2649.1), gest_days = 7 * 39, sex = "M", acronym = "ffmfga", z_or_p = perc)
  testthat_v2x(y = c(2046.7, 2191.8, 2343.5, 2497.5, 2648.1), gest_days = 7 * 40, sex = "F", acronym = "ffmfga", z_or_p = perc)

  # Body fat percentage for gestational age
  testthat_v2x(y = c(16.4, 16.5, 17.15, 17.85, 18.0), gest_days = 7 * 42, sex = "M", acronym = "bfpfga", z_or_p = perc)
  testthat_v2x(y = c(11.6, 12.4, 10.5, 11.8, 13.2), gest_days = 7 * 38, sex = "F", acronym = "bfpfga", z_or_p = perc)
})