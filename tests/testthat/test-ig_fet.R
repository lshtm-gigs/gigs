  ...
  code = {
    ...
    for (fn_suffix in fn_suffixes) {
      ...
      expect_error(
        ...
        regexp = test_error_wrong_type("acronym", "string", "integer")
      )
      # G5.2 --> errors if vector inputs cannot be recycled
      expect_error(
      ...
      # General --> error if `acronym` arg is not scalar
      expect_error(
...
test_that(
  ...
  code = {
    # Make gigs replace bad data with NA and warn the user
...
test_that(
  ...
  code = {
    # Make gigs error if there's bad data
    ...
    for (fn_suffix in fn_suffixes) {
      for (str_acronym in acronyms_to_test) {
        ...
        for (undefined_val in c(NaN, Inf, -Inf)) {
          ...
          expect_error(
            gigs_fn(arg_zyp,
                    replace(arg_x, replace_ints_1, undefined_val)),
            test_msg_undefined(arg_x_name, len_x, num_to_replace1)
          )
        }

        # Bad input 2: Missing data (NA) ---------------------------------------
        ## Throw error for NAs in z/y/p + x variables
        expect_error(
      ...
      expect_error(
        gigs_fn(arg_zyp,
                replace(arg_x, replace_ints_3, NA),
                arg_acronym),
