  ...
  code = {
    ...
    for (fn_suffix in fn_suffixes) {
      ...
      for (seed in c(150, 200, 250)) {
        set.seed(seed)
        acronym <- sample(names(gigs::ig_png), size = 1)
  ...
  code = {
    ...
    for (fn_suffix in fn_suffixes) {
      ...
      for (seed in c(105, 155, 205)) {
        acronym <- sample(names(gigs::ig_png), size = 1)
        x_range <- range(gigs::ig_png[[acronym]][[1]][[1]][[1]])
        ...
        arg_zyp <- switch(fn_suffix,
                          ...
                          rep_len(gigs::ig_png[[acronym]][[1]][[1]][[1]], len_x))
        arg_zyp_name <- switch(fn_suffix, centile2value = "p",
                               zscore2value = "z", "y")
        arg_x <- x
...
msg_xvar_oob <- function(length, int_undefined) {
  paste0("Variable 'x': ", int_undefined, " in ", length, " elements were ",
         "out-of-bounds \\(see the 'ig_png' conversion functions ",
         "documentation\\).")
}
...
#'   be invalid (not in `names(gigs::ig_png)`) in whatever test is calling this
#'   function.
...
msg_acronym_sex_invalid <- function(var, length, int_undefined) {
  see_sentence <- paste0("See the 'ig_png' documentation for valid ",
                           "'acronym' values.")
  if (var == "sex") {
    ...
    for (fn_suffix in fn_suffixes) {
      ...
      for (seed in c(125, 175, 225)) {
        ...
        arg_zyp <- switch(fn_suffix,
                          centile2value = rep_len(p, len_x),
                          zscore2value = rep_len(z, len_x),
                          rep_len(gigs::ig_png[[acronym]][[1]][[1]][[1]],
                                  len_x))
        arg_zyp_name <- switch(fn_suffix, centile2value = "p",
                               zscore2value = "z", "y")
        arg_x <- x
        ...
        # Bad input 5: Invalid `acronym` values (!%in% names(gigs::ig_png)) ----
        expect_warning(
