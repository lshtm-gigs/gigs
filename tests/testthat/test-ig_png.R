...
test_that(
  ...
  code = {
    ...
    # Error message testing: Iterate through ig_png functions, demonstrating
    #   with passed `expect_error()` tests that all error messages are unique
    #   to the data passed in.
    acronyms_to_test <- names(ig_png)
    ...
    for (fn_suffix in fn_suffixes) {
      for (str_acronym in acronyms_to_test) {
        fn_name <- paste("ig_png", str_acronym, fn_suffix, sep = "_")
        gigs_fn <- get(fn_name)
        arg_x_name <- gigs::ig_png[[str_acronym]][["x"]]
        y_name <- gigs::ig_png[[str_acronym]][["y"]]
        ...
        arg_x <- gigs::ig_png[[str_acronym]][[1]][[1]][[1]]
      ...
      fn_name <- paste("ig_png", fn_suffix, sep = "_")
      gigs_fn <- get(fn_name)
      ...
      expect_error(object = gigs_fn(arg_zyp, double(), arg_sex, character()),
                   regexp = test_error_zero_length(c("x")))
      # G5.8b --> incorrect data types
      expect_error(
      ...
      expect_error(
        ...
        regexp = test_error_wrong_type("acronym", "string", "complex")
      )
      # G5.2, --> errors if vector inputs cannot be recycled
      expect_error(
        object = gigs_fn(rep_len(arg_zyp, 2),
      ...
      expect_error(
        ...
        regexp = test_error_acronym_not_scalar(length = 2)
      )
      expect_error(
        object = gigs_fn(arg_zyp,
                         rep_len(arg_x, 2),
                         arg_sex,
                         rep_len(arg_acronym, 2)),
        regexp = test_error_unrecyclable("x")
      )
    }
  }
)
  ...
  code = {
    ...
    for (fn_suffix in fn_suffixes) {
      for (str_acronym in acronyms_to_test) {
        fn_name <- paste("ig_png", str_acronym, fn_suffix, sep = "_")
        gigs_fn <- get(fn_name)
        x <- gigs::ig_png[[str_acronym]][[1]][[1]][[1]]
        len_x <- length(x)
        arg_x_name <- gigs::ig_png[[str_acronym]][["x"]]
        y_name <- gigs::ig_png[[str_acronym]][["y"]]
  ...
  code = {
    ...
    for (fn_suffix in fn_suffixes) {
      for (str_acronym in acronyms_to_test) {
        ...
        len_x <- length(x)
        arg_x_name <- gigs::ig_png[[str_acronym]][["x"]]
        arg_y_name <- gigs::ig_png[[str_acronym]][["y"]]
        ...
        expect_warning(
          ...
          test_msg_xvar_oob(arg_x_name, len_x, num_to_replace2, "ig_png")
        )
        expect_warning(
          gigs_fn(arg_zyp, replace(arg_x, replace_ints_3, out_of_bounds), arg_sex),
          test_msg_xvar_oob(arg_x_name, len_x, num_to_replace3, "ig_png")
        )
        ...
        expect_warning(
          gigs_fn(arg_zyp,
                  arg_x,
                  replace(arg_sex, replace_ints_2, "invalid_sex")),
          test_msg_acronym_sex_invalid("sex", len_x, num_to_replace2, "ig_png")
        )
        expect_warning(
          gigs_fn(arg_zyp,
                  arg_x,
      ...
      expect_warning(
        gigs_fn(arg_zyp,
                ...
                arg_sex,
                arg_acronym),
        test_msg_xvar_oob(arg_x_name, len_x, num_to_replace1, "ig_png")
      )
      ...
      expect_warning(
        gigs_fn(arg_zyp,
                ...
                arg_acronym),
        test_msg_acronym_sex_invalid("sex", len_x, num_to_replace3, "ig_png")
      )
    }
  }
)
...
#'   warnings.
test_that(
  desc = "Invalid values can cause informative errors",
  code = {
    ...
    for (fn_suffix in fn_suffixes) {
      ...
      expect_error(
        gigs_fn(arg_zyp,
                replace(arg_x, replace_ints_1, NA),
                replace(arg_sex, replace_ints_2, values = NA),
                arg_acronym),
        test_msg_missing("sex", len_x, num_to_replace2)
      )
...
#' @srrstats {G5.8c} Show how the ig_png functions handles all-`NA` inputs.
test_that(
  desc = "Test that 'ig_png' functions can handle all-`NA` inputs",
  code = {
    ...
    for (fn_suffix in fn_suffixes) {
      ...
      for (seed in c(125, 175, 225)) {
        str_acronym <- withr::with_seed(seed, code = {
          sample(names(gigs::ig_png), size = 1)
        })
        x <- gigs::ig_png[[str_acronym]][[1]][[1]][[1]]
        len_x <- length(x)
        ...
        expect_error(
          gigs_fn(arg_zyp,
                  arg_x,
                  arg_sex,
                  "not valid acronym!! oh no!"),
          regexp = paste0("Invalid `acronym` value:")
        )
      }
    }
  }
