...
produce a table of differences (see `check_differences()`). Then, using
`diff_equality()`, we'll check that the differences in each row are equal for
all percentiles.
...
Let's check this out for `fmfga_female`:
```{r looking_into_data_3}
check_differences(fmfga_female)
```
The differences in fat mass are equal across percentiles. This hints that to
construct the percentile curves, the mean value ($\mu$) for fat mass was
calculated first, then summed with the product of the desired
$z$-score/percentile with an SD ($\sigma$) that is constant across the
different GAs:
\begin{align*}
y = \mu + z\times\sigma
\end{align*}
...
other_standards <- list(fmfga_male, bfpfga_male, bfpfga_female, ffmfga_male,
                        ffmfga_female)
sapply(X = other_standards, FUN = \(x) all(check_differences(x)$all_equal))
```
...
opposed to a real difference in $\sigma$ for this GA.

## Conclusion
**We need to determine equations for P50 in each standard, as well as the
standard deviation.**

# Extracting models from published tables
## Custom functions
The following custom functions fit models to each INTERGROWTH-21<sup>st</sup>
body composition standard using `lm()`. They then print the adjusted R-squared
for each
...
function does the same, but uses parameters from simpler linear model for each
standard.
...
### Reference vs. predicted values (linear models)
```{r bland_altman_plots2, echo = F, fig.width = 8, fig.height = 12, fig.cap = "Bland-Altman plots for observed and estimated centile values for body composition measures according to gestational age and sex. Circles show differences between observed and estimated values for fat mass (**a** males; **b** females); body fat percentage (**c** males; **d** females); and fat-free mass (**e** males; **f** females) according to the average of the two measurements.", warning = FALSE, message = FALSE}
