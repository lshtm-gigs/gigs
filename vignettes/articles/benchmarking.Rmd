...
We have performed these benchmarks on a Windows 10 system with a Ryzen 7 3700X,
and 16GB of DDR4 RAM. The Stata benchmarks have been run on the same system with
Stata 18.0, using the [benchmark](https://github.com/mcaceresb/stata-benchmark)
package.
...
who_gs_dataset <- function(n, acronym = "wfa") {
  seed <- 154237890
  withr::with_seed(
    seed,
    code = {
...
growthstandards_timings <- mbench_pkg(
  ...
  pkg_name = paste("R: growthstandards", packageVersion(pkg = "growthstandards")))
```
...
This script essentially does the same as `mbench_pkg()`, but for the Stata
package for gigs and the zbmicat package.
```Stata
...
```{r bench_stata, echo = FALSE, eval = FALSE}
