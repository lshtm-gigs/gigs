...
```{r waz_simple_include, fig.align = "center", out.width="95%", echo = FALSE}
...
We start by generating WAZs using each growth standard. To do this, we need to use
...
2. The type of conversion you want to perform:
   *  `value2zscore`
   *  `value2centile`
   *  `zscore2value`
   *  `centile2value`
3. Then use an acronym to tell gigs exactly which growth standard to use. Valid
  acronyms are detailed in the documentation for each conversion function. You
  can provide acronyms either:
   * As an argument within a function call, e.g.
     * `ig_nbs_value2zscore(..., acronym = "wfga")`
     * `ig_png_value2zscore(..., acronym = "wfa")`
     * `who_gs_value2zscore(..., acronym = "wfa")`
   * As part of the function name, e.g.
     * `ig_nbs_wfga_value2zscore()`
     * `ig_png_wfa_value2zscore()`
     * `who_gs_wfa_value2zscore()`

```{r complex_gen_WAZs}
waz_nbs <- with(life6mo, ig_nbs_value2zscore(y = wt_kg,
                                             gest_days = gestage,
                                             sex = sex,
                                             acronym = "wfga"))
waz_who <- with(life6mo, who_gs_value2zscore(y =  wt_kg,
                                             x = age_days,
                                             sex = sex,
                                             acronym = "wfa"))
waz_png <- with(life6mo, ig_png_value2zscore(y =  wt_kg,
                                             x = pma / 7,
                                             sex = sex,
                                             acronym = "wfa"))
```
...
functions to make one overall `waz` column in `life6mo`. The first condition,
`age_days < 0.5`, ensures that the `r intergrowth21st` Newborn Size standards
are used for measures taken <12hrs after birth. The `preterm` and
`pma <= 64 * 7` conditions ensure that the `r intergrowth21st` Postnatal Growth
standards are used for preterm infants who are younger than than 64 weeks PMA
(the upper limit of the `r intergrowth21st` Postnatal Growth standards). The
final condition, for `use_who_gs`, ensures that the WHO Child Growth standard
z-scores are used wherever infants were either born term, or were born preterm
and are older than 64 weeks PMA.

```{r complex_make_WAZ}
is_birth_measure <- with(life6mo_zscored, age_days < 0.5)
...
```{r waz_complex_include, fig.align = "center", out.width="95%", echo = FALSE}
...
```{r sfga_include, fig.align = "center", out.width="95%", echo = FALSE}
...
```{r svn_include, fig.align = "center", out.width="95%", echo = FALSE}
